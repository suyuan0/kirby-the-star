...
onMounted(() => {
  ...
  let datamax = [20, 20, 20, 20, 20, 20, 20, 20, 20];
  let datavaule = [
  ...
  // function contains(arrays: Array<any>, obj: object) {
  //   let i = arrays.length;
  //   while (i--) {
  //     if (arrays[i] === obj) {
  //       return i;
  ...
  let buildSeries = function (data:any) {
    let helper = data.map((item:any, index:number) => {
  ...
  let buildSeries2 = function (data:any) {
    let helper = data.map((item:any, index:any) => {
      let arr = new Array(data.length);
      arr.splice(index, 1, item);
      return arr;
    });
